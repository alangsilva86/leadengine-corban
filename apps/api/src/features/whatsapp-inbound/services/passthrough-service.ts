...
import { PassthroughMessage, findOrCreateOpenTicketByChat, upsertMessageByExternalId } from '@ticketz/storage';

import { prisma } from '../../../lib/prisma';
import { logger } from '../../../config/logger';
import { inboundMessagesProcessedCounter } from '../../../lib/metrics';
import { emitToAgreement, emitToTenant, emitToTicket, getSocketServer } from '../../../lib/socket-registry';
...
export const handlePassthroughIngest = async (event: InboundWhatsAppEvent): Promise<void> => {
  const {
    instanceId,
    contact,
    message,
  ...
  const normalizedPhone =
    sanitizePhone(contactPhone) ?? sanitizePhone(metadataContactPhone) ?? sanitizePhone(metadataRecordPhone);
  ...
  const passthroughDirection = typeof direction === 'string' && direction.toUpperCase() === 'OUTBOUND' ? 'outbound' : 'inbound';
  ...
  const displayName = pickPreferredName(contact.name, contact.pushName, readString(metadataContact.pushName)) ?? 'Contato WhatsApp';
