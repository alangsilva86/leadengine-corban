...
export const readInstanceIdParam = (req: Request): string | null => {
...
export const respondWhatsAppBrokerFailure = (res: Response, error: WhatsAppBrokerError): void => {
  res.status(502).json({
    success: false,
    error: {
...
export const respondLegacyEndpointGone = (res: Response, message: string): void => {
  res.status(410).json({
    success: false,
    error: {
      code: 'ENDPOINT_GONE',
...
export class WhatsAppInstanceInvalidPayloadError extends Error {
  readonly code = 'INVALID_INSTANCE_PAYLOAD';
  readonly status = 400;
...
export const createWhatsAppInstanceSchema = z
  .object({
    name: z
      .string({ required_error: 'Nome da instância é obrigatório.' })
      .trim()
...
type CreateWhatsAppInstanceInput = z.infer<typeof createWhatsAppInstanceSchema>;

type CreateWhatsAppInstanceResult = {
  serialized: ReturnType<typeof serializeStoredInstance>;
  sideEffects: AsyncSideEffect[];
  context: {
    tenantId: string;
    actorId: string;
    instanceId: string;
...
export const collectNumericFromSources = (sources: unknown[], keywords: string[]): number | null => {
...
export const normalizeStatusCountsData = (rawCounts: unknown): Record<string, number> | null => {
  if (!rawCounts) {
    return null;
  }
...
export const normalizeRateUsageData = (rawRate: unknown): {
  used: number;
  limit: number;
  remaining: number;
  percentage: number;
