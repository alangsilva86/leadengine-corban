...
const baseHeaders = () => {
    ...
    if (!tenantId) {
        const envTenant = getEnvVar('VITE_DEFAULT_TENANT_HINT') ??
...
export const buildUrl = (path) => {
    ...
    if (isAbsoluteUrl(API_BASE_URL)) {
        ...
        const windowResult = typeof window !== 'undefined' && window?.location?.origin
        ...
        const relativeResult = attemptBuild(relativeBase, { stripOrigin: true }) ??
    ...
    const execute = async () => {
        ...
        try {
            ...
            if (typeof status === 'number' && (status === 429 || status === 503 || status >= 500)) {
                const retryAfterMs = parseRetryAfterMs(error?.retryAfter ?? error?.payload?.retryAfter ?? error?.rateLimitDelayMs ?? null);
                const attempt = bucket.attempts + 1;
                bucket.attempts = attempt;
                const waitDuration = retryAfterMs !== null
                    ? retryAfterMs
