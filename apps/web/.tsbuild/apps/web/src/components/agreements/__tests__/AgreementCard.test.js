...
describe('AgreementCard', () => {
    ...
    it('renders agreement information', () => {
        render(_jsx(AgreementCard, { ...baseProps }));
        ...
        expect(screen.getByText(/Atualizado em/)).toHaveTextContent(`Atualizado em ${new Date(baseProps.lastSyncAt).toLocaleString()}`);
    });
    it('highlights the selected state', () => {
        render(_jsx(AgreementCard, { ...baseProps, isSelected: true }));
        const button = screen.getByRole('button', { name: /Convênio selecionado/i });
        expect(button).toBeInTheDocument();
    });
    it('triggers selection handler', () => {
        const onSelect = vi.fn();
        render(_jsx(AgreementCard, { ...baseProps, onSelect: onSelect }));
...
describe('AgreementCardSkeleton', () => {
    ...
    it('renders skeleton placeholders', () => {
        const { container } = render(_jsx(AgreementCardSkeleton, {}));
