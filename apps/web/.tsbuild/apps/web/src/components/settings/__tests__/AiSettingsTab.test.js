...
const buildFetchMock = () => {
    ...
    const fetchMock = vi.fn(async (input, init) => {
        ...
        return new Response(JSON.stringify({ success: false, message: 'Endpoint not mocked' }), { status: 404 });
    });
    return fetchMock;
};
describe('AiSettingsTab', () => {
    ...
    it('carrega configurações e permite alterar o modo padrão', async () => {
        ...
        await waitFor(() => {
            const putCall = fetchMock.mock.calls.find(([input, init]) => typeof input === 'string' &&
