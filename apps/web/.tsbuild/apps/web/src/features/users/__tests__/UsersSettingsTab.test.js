...
const renderTab = () => {
    ...
    return render(_jsx(QueryClientProvider, { client: client, children: _jsx(UsersSettingsTab, {}) }));
};
describe('UsersSettingsTab', () => {
    ...
    beforeEach(() => {
        ...
        global.fetch = apiMock.fetchMock;
    });
    afterAll(() => {
    ...
    it('atualiza a função direto na tabela', async () => {
        ...
        expect(JSON.parse((patchCall?.[1]?.body ?? '{}'))).toMatchObject({ role: 'ADMIN' });
    });
});
