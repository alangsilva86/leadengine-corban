...
describe('CampaignsPanel', () => {
    ...
    it('exibe convênio, estado de vínculo e permite desvincular a instância', async () => {
        ...
        render(_jsx(CampaignsPanel, { agreementName: "Conv\u00EAnio Alpha", campaigns: [linkedCampaign, awaitingCampaign], loading: false, error: null, onRefresh: vi.fn(), onCreateClick: vi.fn(), onPause: vi.fn(), onActivate: vi.fn(), onDelete: vi.fn(), onReassign: vi.fn(), onDisconnect: disconnectMock, actionState: null, selectedInstanceId: "instance-1", canCreateCampaigns: true }));
        ...
        expect(await screen.findByRole('menuitem', { name: /Vincular instância/i })).toBeInTheDocument();
    });
    it('permite atualizar a lista e desabilita criação quando necessário', async () => {
        ...
        const user = userEvent.setup();
        const { rerender, container } = render(_jsx(CampaignsPanel, { agreementName: "Conv\u00EAnio Controlado", campaigns: campaigns, loading: false, error: null, onRefresh: onRefresh, onCreateClick: onCreateClick, onPause: vi.fn(), onActivate: vi.fn(), onDelete: vi.fn(), onReassign: vi.fn(), onDisconnect: vi.fn(), actionState: null, selectedInstanceId: "instance-1", canCreateCampaigns: false }));
        ...
        rerender(_jsx(CampaignsPanel, { agreementName: "Conv\u00EAnio Controlado", campaigns: campaigns, loading: true, error: null, onRefresh: onRefresh, onCreateClick: onCreateClick, onPause: vi.fn(), onActivate: vi.fn(), onDelete: vi.fn(), onReassign: vi.fn(), onDisconnect: vi.fn(), actionState: null, selectedInstanceId: "instance-1", canCreateCampaigns: false }));
