...
const persistShowAllPreference = (value) => {
    try {
...
const initialState = (status, activeCampaign) => ({
...
const useWhatsAppConnect = ({ selectedAgreement, status = 'disconnected', activeCampaign, onboarding, onStatusChange, onCampaignReady, onBack, onContinue, }) => {
    ...
    const setQrPanelOpen = useCallback((value) => {
        dispatch({ type: 'set-qr-panel-open', value });
    }, []);
    const setQrDialogOpen = useCallback((value) => {
        dispatch({ type: 'set-qr-dialog-open', value });
    }, []);
    const setInstancePendingDelete = useCallback((value) => {
        dispatch({ type: 'set-instance-pending-delete', value });
    }, []);
    const setCreateInstanceOpen = useCallback((value) => {
        dispatch({ type: 'set-create-instance-open', value });
    }, []);
    const setPairingPhoneInput = useCallback((value) => {
        dispatch({ type: 'set-pairing-phone-input', value });
    }, []);
    const setPairingPhoneError = useCallback((value) => {
        dispatch({ type: 'set-pairing-phone-error', value });
    }, []);
    const setRequestingPairing = useCallback((value) => {
        dispatch({ type: 'set-requesting-pairing', value });
    }, []);
    const setExpandedInstanceId = useCallback((value) => {
    ...
    const { campaign, campaigns, campaignsLoading, campaignError, campaignAction, createCampaign, updateCampaignStatus, deleteCampaign, reassignCampaign, reloadCampaigns, fetchCampaignImpact, setCreateCampaignOpen, isCreateCampaignOpen, setPendingReassign, pendingReassign, setReassignIntent, reassignIntent, persistentWarning, clearCampaign, } = campaignState;
    const { statusCopy: statusCopyData, statusTone, countdownMessage, qrImageSrc, isGeneratingQrImage, qrStatusMessage, isBusy, canContinue, qrPanelOpen, isQrDialogOpen, handleViewQr, handleGenerateQr, handleMarkConnected, pairingPhoneInput, pairingPhoneError, requestingPairingCode, handlePairingPhoneChange, handleRequestPairingCode, } = sessionUiState;
    const { instance, renderInstances, instancesReady, hasHiddenInstances, visibleInstanceCount, totalInstanceCount, tenantScopeNotice, tenantFilterId, tenantFilterLabel, tenantFilteredOutCount, selectedInstanceBelongsToTenant, tenantScopedInstances, localStatus, qrData, loadingInstances, loadingQr, isAuthenticated, deletingInstanceId, liveEvents, loadInstances, selectInstance, generateQr, connectInstance, createInstance, deleteInstance, markConnected, handleAuthFallback, setSecondsLeft, setGeneratingQrState, setInstanceStatus, realtimeConnected, selectedInstanceStatus, showAllInstances, setShowAllInstances, createInstanceWarning, canCreateInstance, nextInstanceOrdinal, } = tenantState;
    useEffect(() => {
    ...
    const onboardingDescription = '1. Conecte seus números ao Lead Engine. 2. Vincule origens comerciais (convênios, parceiros ou filas) quando fizer sentido. 3. Ative campanhas apenas se precisar de roteamento avançado.';
    ...
    const hasConnectedInstances = renderInstances.some(isInstanceConnected) ||
        (selectedInstanceBelongsToTenant && instance ? isInstanceConnected(instance) : false);
    const connectionHealthy = Boolean(instance) &&
    ...
    const instanceViewModels = useMemo(() => {
    ...
    const submitCreateInstance = useCallback(async ({ name, id }) => {
        if (!canCreateInstance) {
            const warningMessage = createInstanceWarning ??
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : 'Não foi possível criar uma nova instância';
            setErrorMessage(message);
    ...
    const handleInstanceSelect = useCallback(async (inst, { skipAutoQr = false } = {}) => {
    ...
    const handleDeleteInstance = useCallback(async (target) => {
        if (!target?.id) {
            return;
        }
    ...
    const removalTargetLabel = state.instancePendingDelete?.name ||
        state.instancePendingDelete?.displayId ||
        state.instancePendingDelete?.id ||
        'selecionada';
    const removalKind = state.instancePendingDelete?.kind || (state.instancePendingDelete?.isSession ? 'session' : null);
