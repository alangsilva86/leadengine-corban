...
const buildViewModel = (overrides = {}) => ({
    ...
    rateUsage: overrides.rateUsage ?? { used: 50, remaining: 50, limit: 100, percentage: 50 },
...
describe('InstancesPanel', () => {
    ...
    it('renderiza instâncias disponíveis com resumo operacional e ação principal', async () => {
        ...
        render(_jsx(InstancesPanel, { ...baseProps }));
        ...
        expect(baseProps.onSelectInstance).toHaveBeenCalledWith(expect.objectContaining({ id: 'instance-1' }), expect.objectContaining({ skipAutoQr: true }));
    });
    it('exibe estado vazio quando não há instâncias renderizáveis mas existem ocultas', async () => {
        const user = userEvent.setup();
        render(_jsx(InstancesPanel, { ...baseProps, instancesReady: true, hasRenderableInstances: false, hasHiddenInstances: true, instanceViewModels: [] }));
    ...
    it('exibe estado de erro com ação de tentar novamente', async () => {
        const user = userEvent.setup();
        render(_jsx(InstancesPanel, { ...baseProps, errorState: { title: 'Erro', message: 'Algo deu errado' } }));
