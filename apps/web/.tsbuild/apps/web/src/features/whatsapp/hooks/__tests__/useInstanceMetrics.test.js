...
import useInstanceMetrics, { categorizeHealth, computeHealthScore, computeLoadLevel, resolveProvider, resolveTimestamp, } from '../useInstanceMetrics.js';
describe('useInstanceMetrics', () => {
    ...
    it('aplica filtros de status, saúde e provider mantendo opções ordenadas', () => {
        ...
        expect(result.current.enrichedInstances.find((item) => item.instance.id === 'b')?.connectionState).toBe('reconnecting');
    ...
    it('ordena por atualização e carga, calculando sumário e prioridade', () => {
        ...
        const { result } = renderHook(() => useInstanceMetrics({ instanceViewModels: [vmA, vmB, vmC], instancesReady: true }));
