...
describe('useInstanceLiveUpdates', () => {
    it('conecta ao socket, ingressa no tenant e encaminha eventos', async () => {
        const onEvent = vi.fn();
        const { result, unmount } = renderHook(() => useInstanceLiveUpdates({ tenantId: 'tenant-123', enabled: true, onEvent }));
        await waitFor(() => {
    ...
    it('reporta mensagens de erro quando a conexão falha imediatamente', async () => {
        ...
        const { result } = renderHook(() => useInstanceLiveUpdates({ tenantId: 'tenant-456', enabled: true }));
        await waitFor(() => {
