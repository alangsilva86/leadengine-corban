...
const asRecord = (value) => (isPlainObject(value) ? value : undefined);
...
export const pickMetric = (source, keys) => {
    ...
    while (stack.length > 0) {
        ...
        for (const [propKey, propValue] of Object.entries(current)) {
            ...
            const fuzzyMatch = normalizedTargets.some((target) => target.length > 0 && normalizedKey.includes(target));
...
const DEFAULT_STATUS_KEYS = ['1', '2', '3', '4', '5'];
...
export const normalizeStatusCounts = (rawCounts) => {
    ...
    return DEFAULT_STATUS_KEYS.reduce((acc, key, index) => {
        ...
        return acc;
    }, Object.assign({}, ...DEFAULT_STATUS_KEYS.map((key) => ({ [key]: 0 }))));
};
...
export const normalizeRateUsage = (rawRate) => {
    ...
    const usedCandidate = toNumber(pickMetric(rawRate, ['usage', 'used', 'current', 'value', 'count', 'consumed']));
    const limitCandidate = toNumber(pickMetric(rawRate, ['limit', 'max', 'maximum', 'quota', 'total', 'capacity']));
    const remainingCandidate = toNumber(pickMetric(rawRate, ['remaining', 'left', 'available', 'saldo', 'restante']));
...
export const getInstanceMetrics = (instance) => {
    ...
    const metricsSource = mergeMetricsSources(instanceRecord.metrics, instanceRecord.stats, instanceRecord.messages, instanceRecord.rawStatus, instanceRecord);
    ...
    const statusCountsSource = findStatusCountsSource(metricsSource) ??
