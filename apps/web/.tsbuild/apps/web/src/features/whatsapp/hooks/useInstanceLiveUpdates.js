            ...
            try {
                ...
                const handleConnected = () => {
                    ...
                    socket.emit('join-tenant', { tenantId }, (ack) => {
                        if (!ack || ack.ok !== true) {
                            setConnectionError((ack && ack.error) || 'Falha ao ingressar na sala do tenant');
                        }
                    });
                };
                ...
                const handleConnectError = (error) => {
                    if (!isActive)
                        return;
                    const message = error instanceof Error ? error.message : 'Falha ao conectar no tempo real';
                    setConnectionError(message);
                };
                const handleReconnectFailed = () => {
                    if (!isActive)
                        return;
                    setConnectionError('Não foi possível restabelecer conexão em tempo real após várias tentativas.');
                };
                ...
                if (!isActive)
                    return;
                const message = error instanceof Error ? error.message : 'Falha ao carregar socket.io-client';
                setConnectionError(message);
            }
        };
        connect();
