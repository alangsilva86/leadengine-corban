export const createMutationsSlice = (events, set, get) => ({
    ...
    markConnected(payload) {
        ...
        set((prev) => ({
            instances: prev.instances.map((item) => item.id === payload.instanceId
                ? { ...item, status: payload.status, connected: payload.status === 'connected' }
                : item),
            currentInstance: prev.currentInstance && prev.currentInstance.id === payload.instanceId
                ? {
