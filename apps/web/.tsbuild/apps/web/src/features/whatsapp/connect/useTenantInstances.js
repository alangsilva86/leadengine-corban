...
const useTenantInstances = ({ selectedAgreement, status, activeCampaign, onStatusChange, onError, logger, dispatch, state, campaignInstanceId, }) => {
    const { instances, instancesReady, currentInstance: instance, status: rawStatus, qrData, secondsLeft, loadingInstances, loadingQr, isAuthenticated, deletingInstanceId, liveEvents, loadInstances, selectInstance, generateQr, connectInstance, createInstance, deleteInstance, markConnected, handleAuthFallback, setSecondsLeft, setGeneratingQrState, setStatus, realtimeConnected, selectedInstanceStatus, } = useWhatsAppInstances({
        selectedAgreement,
        status,
        onStatusChange,
        onError,
    ...
    const tenantFilterId = useMemo(() => resolveAgreementTenantId(selectedAgreement), [selectedAgreement]);
    const tenantFilterLabel = useMemo(() => resolveTenantDisplayName(selectedAgreement), [selectedAgreement]);
    ...
    const setShowAllInstances = useCallback((value) => {
        dispatch({ type: 'set-show-all-instances', value });
    }, [dispatch]);
    const visibleInstances = useMemo(() => tenantScopedInstances.filter(shouldDisplayInstance), [tenantScopedInstances]);
    ...
    const renderInstances = useMemo(() => sortInstancesByLabel(filteredInstances), [filteredInstances]);
    const tenantScopeNotice = tenantFilterId && tenantFilteredOutCount > 0
        ? `${tenantFilteredOutCount} instância(s) ocultadas por pertencerem a tenants diferentes de ${tenantFilterLabel ?? tenantFilterId}.`
        : null;
