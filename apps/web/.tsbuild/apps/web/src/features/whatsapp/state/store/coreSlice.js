import { selectPreferredInstance, shouldDisplayInstance, } from '../../lib/instances';
...
const hasConfigChanged = (current, next) => {
...
const updateCache = (deps, list, currentId) => {
    deps.persistCache(list, currentId);
};
export const createCoreSlice = (set, get, events, deps) => ({
    instances: [],
    ...
    hydrateFromCache() {
        ...
        const list = Array.isArray(cache.list)
            ? cache.list.filter(Boolean)
            : [];
        const current = cache.currentId && list.length
    ...
    applyLoadResult(payload, meta) {
        ...
        const nextCurrent = selectPreferredInstance(list, selectionOptions) ?? payload.instance ?? null;
    ...
    setInstances(list, options = {}) {
        ...
        const current = selectPreferredInstance(filtered, options) ??
            get().currentInstance ??
            filtered[0] ??
            null;
        set({
    ...
    updateInstance(instanceId, updates) {
        ...
        set((state) => {
            const instances = state.instances.map((item) => item.id === instanceId ? { ...item, ...updates } : item);
            const current = state.currentInstance && state.currentInstance.id === instanceId
