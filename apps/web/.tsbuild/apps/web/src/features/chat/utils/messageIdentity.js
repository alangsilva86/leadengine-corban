...
export const resolveProviderMessageId = (entry) => {
    ...
    const topLevelCandidate = typeof entry.providerMessageId === 'string' && entry.providerMessageId.trim().length > 0
    ...
    const candidate = [broker.messageId, broker.id, broker.wamid].find((value) => typeof value === 'string' && value.trim().length > 0);
