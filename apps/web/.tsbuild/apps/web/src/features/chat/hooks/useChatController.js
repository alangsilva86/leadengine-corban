...
export const useChatController = ({ tenantId, currentUser } = {}) => {
    ...
    const apiFilters = useMemo(() => buildApiFilters({ filters, currentUser }), [filters, currentUser]);
    ...
    const selectedTicket = useMemo(() => findTicketById(tickets, selectedTicketId), [selectedTicketId, tickets]);
    ...
    const handleMessageCreated = useCallback((payload) => {
        ...
        queryClient.setQueryData(queryKey, (current) => {
            ...
            const alreadyExists = current.pages.some((page) => Array.isArray(page?.items) &&
    ...
    const handleQueueMissing = useCallback((payload) => {
        setQueueAlerts((current) => {
            ...
            const filtered = current.filter((entry) => entry.payload?.instanceId !== payload?.instanceId &&
    ...
    useEffect(() => {
        ...
        const interval = window.setInterval(() => {
            setQueueAlerts((current) => current.filter((entry) => Date.now() - entry.timestamp < 5 * 60 * 1000));
        }, 60 * 1000);
