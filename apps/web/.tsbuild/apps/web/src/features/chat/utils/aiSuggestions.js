...
const coerceObjectLine = (value, { titleKey = 'title', bodyKey = 'message' } = {}) => {
    ...
    const title = typeof value[titleKey] === 'string' && value[titleKey].trim().length > 0
        ? value[titleKey].trim()
        : null;
    const body = typeof value[bodyKey] === 'string' && value[bodyKey].trim().length > 0
        ? value[bodyKey].trim()
        : null;
    if (title && body) {
        return `${title}: ${body}`;
...
export const extractAiSuggestion = (rawPayload = {}) => {
    ...
    const confidence = normalizeConfidence(payload?.confidence ?? payload?.confidence_score ?? suggestionRoot?.confidence ?? null);
    ...
    const fallbackNotes = !nextStep && combinedTips.length === 0 && objections.length === 0
        ? [
