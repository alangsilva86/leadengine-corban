...
describe('useRealtimeTickets', () => {
    ...
    it("invokes onTyping when receiving a 'ticket:typing' event", async () => {
        ...
        const { unmount } = renderHook(() => useRealtimeTickets({ enabled: true, tenantId: 'tenant-123', onTyping }));
        await waitFor(() => {
    ...
    it('avoids duplicate callbacks when message updates use aliased events', async () => {
        ...
        const { unmount } = renderHook(() => useRealtimeTickets({ enabled: true, tenantId: 'tenant-123', onMessageUpdated }));
        await waitFor(() => {
        ...
        const messageUpdateLogs = logSpy.mock.calls.filter(([message]) => typeof message === 'string' && message.includes('Mensagem atualizada'));
