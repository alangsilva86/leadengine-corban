...
describe('ManualConversationDialog', () => {
    ...
    it('renderiza apenas instâncias conectadas no seletor', async () => {
        ...
        render(_jsx(ManualConversationDialog, { open: true, onOpenChange: () => { }, onSubmit: vi.fn(), onSuccess: vi.fn() }));
    ...
    it('envia o instanceId selecionado no payload', async () => {
        ...
        render(_jsx(ManualConversationDialog, { open: true, onOpenChange: () => { }, onSubmit: onSubmit, onSuccess: onSuccess }));
        const user = userEvent.setup();
    ...
    it('mostra mensagens de erro de validação quando campos obrigatórios estão vazios', async () => {
        ...
        render(_jsx(ManualConversationDialog, { open: true, onOpenChange: () => { }, onSubmit: onSubmit, onSuccess: vi.fn() }));
        const user = userEvent.setup();
        await user.click(screen.getByRole('button', { name: /iniciar conversa/i }));
    ...
    it('exibe o erro retornado pela mutação', () => {
        render(_jsx(ManualConversationDialog, { open: true, onOpenChange: () => { }, onSubmit: vi.fn(), onSuccess: vi.fn(), error: new Error('Falha ao iniciar a conversa') }));
