...
import { formatStageLabel, getLegacyStageValue, getStageValue, normalizeStage as normalizeSalesStage, } from '../components/ConversationArea/utils/stage.js';
const asDate = (value) => {
    if (!value)
        return null;
...
const buildTimeline = ({ messages, ticket, notes }) => {
    ...
    messages.forEach((message, index) => {
        ...
        const canonicalKey = resolveMessageKey(message) ??
    ...
    if (Array.isArray(ticket?.salesTimeline)) {
        ticket.salesTimeline.forEach((event, index) => {
            ...
            const stageValue = normalizedStage !== 'DESCONHECIDO'
                ? getStageValue(normalizedStage)
                : null;
            const legacyStageValue = normalizedStage !== 'DESCONHECIDO'
...
export const useConversationState = ({ ticket, messagesPages, notes }) => {
    ...
    const timeline = useMemo(() => buildTimeline({ messages, ticket, notes }), [messages, ticket, notes]);
