...
const useAiControlPanel = ({ ticket, aiMode, aiConfidence, onAiModeChange, onTakeOver, onGiveBackToAi, aiModeChangeDisabled = false, }) => {
    ...
    const aiConfidencePercent = useMemo(() => (normalizedConfidence !== null ? Math.round(normalizedConfidence * 100) : null), [normalizedConfidence]);
    ...
    const handleAiModeSelect = useCallback((value) => {
    ...
    const takeoverDisabled = useMemo(() => !ticket || !onTakeOver || aiModeChangeDisabled || normalizedAiMode === 'manual', [aiModeChangeDisabled, normalizedAiMode, onTakeOver, ticket]);
    const giveBackDisabled = useMemo(() => !ticket ||
        !onGiveBackToAi ||
        aiModeChangeDisabled ||
        normalizedAiMode === 'auto' ||
        normalizedConfidence === null ||
        normalizedConfidence < AI_HANDOFF_CONFIDENCE_THRESHOLD, [aiModeChangeDisabled, normalizedAiMode, normalizedConfidence, onGiveBackToAi, ticket]);
    const takeoverTooltipMessage = useMemo(() => {
        if (!ticket)
            return 'Nenhum ticket selecionado';
        if (!onTakeOver)
            return 'Ação indisponível';
        if (aiModeChangeDisabled)
            return 'Aguardando estado da IA';
        if (normalizedAiMode === 'manual')
            return 'Agente já está no comando';
    ...
    const giveBackTooltipMessage = useMemo(() => {
        ...
        if (aiModeChangeDisabled)
            return 'Aguardando estado da IA';
        if (normalizedAiMode === 'auto')
            return 'IA já está no comando';
        if (normalizedConfidence === null)
            return 'Confiança da IA indisponível';
...
export { AI_MODE_OPTIONS, DEFAULT_AI_MODE, AI_HANDOFF_CONFIDENCE_THRESHOLD, AI_CONFIDENCE_TONES, isValidAiMode, };
