...
export const useConversationExperience = ({ ticket, conversation, messagesQuery, typingIndicator, onSendMessage, onCreateNote, onSendTemplate, onCreateNextStep, onRegisterResult, onRegisterCallResult, onAssign, onGenerateProposal, onScheduleFollowUp, onSendSMS, onEditContact, onContactFieldSave = async () => { }, onDealFieldSave = async () => { }, nextStepValue = '', onNextStepSave = async () => { }, isRegisteringResult = false, currentUser = null, isSending = false, sendError = null, composerDisabled = false, composerDisabledReason = null, aiMode, aiConfidence, aiModeChangeDisabled = false, onAiModeChange, onTakeOver, onGiveBackToAi, composerNotice: composerNoticeProp = null, }) => {
    ...
    const { timelineItems: messageTimelineItems, hasMore, isLoadingMore, handleLoadMore, lastEntryKey: messageLastEntryKey, } = useTicketMessages(messagesQuery);
    ...
    const { instances: whatsappInstances = [], loadingInstances = false, loadInstances, instancesReady = false, } = useWhatsAppInstances();
    ...
    const instanceOptions = useMemo(() => {
        ...
        const options = whatsappInstances
            ...
            .map((instance) => {
            const status = typeof instance.status === 'string' && instance.status.trim().length > 0
            ...
            const primaryLabel = typeof instance.name === 'string' && instance.name.trim().length > 0
                ? instance.name.trim()
                : typeof instance.displayId === 'string' && instance.displayId.trim().length > 0
                    ? instance.displayId.trim()
                    : instance.id;
            const phone = typeof instance.phoneNumber === 'string' && instance.phoneNumber.trim().length > 0
    ...
    useEffect(() => {
        ...
        if (selectedInstanceId) {
            ...
            if (!exists) {
                const fallback = instanceOptions.find((option) => option.id === defaultInstanceId) ??
        ...
        const preferred = (defaultInstanceId &&
    ...
    const selectedInstanceOption = useMemo(() => instanceOptions.find((option) => option.id === selectedInstanceId) ?? null, [instanceOptions, selectedInstanceId]);
    const defaultInstanceOption = useMemo(() => instanceOptions.find((option) => option.id === defaultInstanceId) ?? null, [defaultInstanceId, instanceOptions]);
    ...
    const instanceNotice = useMemo(() => {
        ...
        if (selectionDiffersFromDefault && selectedInstanceOption && selectedInstanceOption.connected) {
            const defaultLabel = defaultInstanceOption?.label ??
    ...
    const selectedInstanceConnected = selectedInstanceOption?.connected ?? instanceOptions.length === 0;
    const composerInstanceSelector = useMemo(() => ({
        options: instanceOptions,
        selectedId: selectedInstanceOption?.id ?? null,
        selectedLabel: selectedInstanceOption?.label ?? selectedInstanceOption?.description ?? null,
        ...
        defaultLabel: defaultInstanceOption?.label ?? defaultInstanceOption?.description ?? null,
    ...
    const handleComposerSend = useCallback((payload) => {
        const basePayload = payload && typeof payload === 'object' ? { ...payload } : {};
        const resolvedInstanceId = selectedInstanceOption?.id ?? defaultInstanceId ?? null;
        const resolvedInstanceLabel = selectedInstanceOption?.label ??
    ...
    const handleComposerTemplate = useCallback((template) => {
        if (!template)
            return;
        const text = template.body ?? template.content ?? template;
        onSendMessage?.({ content: text, template });
    }, [onSendMessage]);
    const handleComposerCreateNote = useCallback((note) => {
        onCreateNote?.(note);
    }, [onCreateNote]);
    ...
    const aiState = useMemo(() => {
        ...
        const confidenceValue = typeof suggestion?.confidence === 'number'
    ...
    const requestAiSuggestions = useCallback((payload = {}) => {
    ...
    const aiContextTimeline = useMemo(() => buildAiContextTimeline(combinedTimelineItems), [combinedTimelineItems]);
    ...
    useEffect(() => {
        if (!composerApiRef.current?.setDraftValue ||
    ...
    const headerProps = useMemo(() => ({
        ...
        aiConfidence: typeof aiConfidence === 'number' && Number.isFinite(aiConfidence)
            ? aiConfidence
