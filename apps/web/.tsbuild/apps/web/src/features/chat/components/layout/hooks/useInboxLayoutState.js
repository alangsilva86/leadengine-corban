...
import { CONTEXT_PREFERENCE_KEY, readPreference, writePreference, } from '../preferences.ts';
const useInboxLayoutState = ({ defaultContextOpen = false, contextAvailable = false, currentUser, telemetry = emitInboxTelemetry, } = {}) => {
    const isTablet = useMediaQuery('(min-width: 1024px)');
    const isDesktop = useMediaQuery('(min-width: 1280px)');
    const shouldRenderSplitLayout = isTablet;
    const canPersistPreferences = Boolean(currentUser?.id);
    const [contextOpen, setContextOpen] = useState(() => canPersistPreferences
