...
const isMeaningfulDisplayName = (value) => {
    ...
    if (!trimmed || trimmed.length === 1)
        return false;
    if (isPlaceholderContactLabel(trimmed))
        return false;
    if (looksLikeWhatsAppJid(trimmed))
        return false;
...
export const getTicketIdentity = (ticket) => {
    ...
    const remoteJidCandidate = pickString(metadataContact.remoteJid, whatsappMeta.remoteJid, metadata.remoteJid, timelineMeta.remoteJid) || null;
    const phoneCandidate = pickString(contact.phone, contact.primaryPhone, contactMeta.phone, metadata.contactPhone, metadataContact.phone, metadataContact.msisdn, metadataContact.address, whatsappMeta.phone, whatsappMeta.msisdn, metadataLead.phone, remoteJidCandidate);
    const sanitizedPhone = sanitizePhone(phoneCandidate);
    const displayPhone = sanitizedPhone ? formatPhoneNumber(sanitizedPhone) : null;
    const remoteIdentifier = remoteJidCandidate && remoteJidCandidate.includes('@')
    ...
    const secondaryName = meaningfulName ??
        normalizedNameCandidates.find((candidate) => !looksLikeWhatsAppJid(candidate) && !isPlaceholderContactLabel(candidate)) ??
        null;
    const fallbackName = secondaryName ??
        (sanitizedPhone ?? null) ??
