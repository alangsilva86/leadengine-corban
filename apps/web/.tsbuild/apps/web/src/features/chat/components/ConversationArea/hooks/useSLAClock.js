...
export const useSLAClock = (ticket) => {
    return useMemo(() => {
        ...
        const elapsedMs = durationMs !== null && typeof remainingMs === 'number' ? Math.max(durationMs - remainingMs, 0) : null;
        const progress = durationMs && durationMs > 0 && elapsedMs !== null ? Math.min(Math.max((elapsedMs / durationMs) * 100, 0), 100) : null;
        return {
            deadline,
            remainingMs,
            durationMs,
