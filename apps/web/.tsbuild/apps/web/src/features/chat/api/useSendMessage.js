...
const normalizeOutboundErrorDetails = (errorLike, fallbackMessage) => {
    ...
    const rawMessage = typeof source.message === 'string' && source.message.trim().length > 0
        ? source.message.trim()
        : typeof fallbackMessage === 'string' && fallbackMessage.trim().length > 0
            ? fallbackMessage.trim()
            : 'Não foi possível enviar a mensagem.';
    const normalizedCode = typeof source.code === 'string' && source.code.trim().length > 0
        ? source.code.trim().toUpperCase()
        : null;
    const normalizedRequestId = typeof source.requestId === 'string' && source.requestId.trim().length > 0
        ? source.requestId.trim()
        : null;
    const normalizedRecoveryHint = typeof source.recoveryHint === 'string' && source.recoveryHint.trim().length > 0
        ? source.recoveryHint.trim()
        : null;
    const normalizedStatus = typeof source.status === 'number'
        ? source.status
...
const ensureErrorPayload = (errorLike, normalizedError) => {
    ...
    const payload = errorLike.payload && typeof errorLike.payload === 'object' && errorLike.payload !== null
        ? errorLike.payload
        : {};
    payload.error = normalizedError;
...
const buildPayload = ({ type, content, mediaUrl, mediaMimeType, mediaFileName, caption, }) => {
...
export const useSendMessage = ({ fallbackTicketId } = {}) => {
    ...
    return useMutation({
        ...
        mutationFn: async ({ ticketId, content, type = 'TEXT', mediaUrl, mediaMimeType, mediaFileName, caption, quotedMessageId, metadata: _metadata, instanceId, }) => {
            ...
            for (let attempt = 0; attempt < MAX_RETRY_ATTEMPTS; attempt += 1) {
                try {
                    ...
                    const response = await apiPost(`/api/tickets/${targetTicketId}/messages`, {
                    ...
                    const normalizedError = normalizeOutboundErrorDetails(error?.payload?.error ?? error, error?.message);
                    ...
                    const status = typeof enrichedError?.status === 'number'
