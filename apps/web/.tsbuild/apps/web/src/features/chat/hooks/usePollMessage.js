...
import { POLL_PLACEHOLDER_MESSAGES, getFirstInteger, getFirstNonEmptyString, } from '@ticketz/shared';
const buildPollOptionsLookup = (metadataPoll, pollChoiceMetadata, interactivePoll) => {
    ...
    const register = (option, index) => {
        ...
        const labelCandidate = getFirstNonEmptyString(option.title, option.text, option.name, option.description, option.optionName, option.label, option.displayName);
...
const buildSelectedOptions = (metadataPoll, pollChoiceMetadata, pollOptionsLookup) => {
    ...
    const pushSelection = (id, title, indexHint, labelFallback) => {
        ...
        const resolvedIndex = typeof indexHint === 'number'
            ? indexHint
            : lookupById?.index ?? lookupByIndex?.index ?? selections.length;
        const resolvedTitle = normalizedTitleCandidate ??
            lookupById?.label ??
            lookupByIndex?.label ??
    ...
    fromMetadata.forEach((selection, index) => {
        ...
        const titleCandidate = getFirstNonEmptyString(selection.title, selection.text, selection.name, selection.optionName, selection.label, selection.description, selection.displayName);
    ...
    if (selections.length === 0) {
        ...
        optionIds.forEach((optionId, index) => {
            ...
            pushSelection(normalizedId, lookupEntry?.label ?? normalizedId, lookupEntry?.index ?? index, lookupEntry?.label ?? normalizedId);
        });
    }
...
const normalizePollOptions = (optionsSource, pollOptionsLookup, pollOptionTotals, pollSelectedIdSet, pollSelectedTitleSet) => {
    ...
    return optionsSource.map((option, index) => {
        ...
        if (typeof option === 'string') {
            ...
            const lookupEntry = (normalizedId ? pollOptionsLookup.get(normalizedId) : null) ?? pollOptionsLookup.get(`__index:${index}`);
            const label = normalizedId || lookupEntry?.label || `Opção ${index + 1}`;
            const votes = normalizedId && pollOptionTotals && typeof pollOptionTotals === 'object'
                ? pollOptionTotals[normalizedId] ?? null
                : null;
            const isSelected = (normalizedId && pollSelectedIdSet.has(normalizedId)) ||
        ...
        const lookupEntry = (normalizedId ? pollOptionsLookup.get(normalizedId) : null) ?? pollOptionsLookup.get(`__index:${normalizedIndex}`);
        const labelCandidate = getFirstNonEmptyString(option.title, option.text, option.name, option.description, option.optionName, option.label, option.displayName, lookupEntry?.label);
        const label = labelCandidate ?? (normalizedId ? normalizedId : `Opção ${normalizedIndex + 1}`);
        const votesFromOption = typeof option.votes === 'number' ? option.votes : typeof option.count === 'number' ? option.count : null;
        const votesFromTotals = normalizedId && pollOptionTotals && typeof pollOptionTotals === 'object'
            ? pollOptionTotals[normalizedId] ?? null
            : null;
...
export const usePollMessage = ({ message, messageType, rawTextContent }) => useMemo(() => {
    ...
    const pollLikeMetadata = Boolean(pollMetadata?.origin === 'poll_choice' || metadataPoll || pollChoiceMetadata || interactivePoll);
    const hasMeaningfulText = trimmedRawText.length > 0 && !POLL_PLACEHOLDER_MESSAGES.has(trimmedRawText);
    const shouldForceText = hasMeaningfulText &&
    ...
    const pollFallbackText = pollSelectedOptions.length > 0
        ? pollSelectedOptions
            .map((selection) => (typeof selection.title === 'string' ? selection.title.trim() : null))
            .filter(Boolean)
            .join(', ')
        : null;
    const textContent = pollFallbackText && trimmedRawText && POLL_PLACEHOLDER_MESSAGES.has(trimmedRawText)
        ? pollFallbackText
        : rawText;
    ...
    const pollSelectedIdSet = new Set(pollSelectedOptions
        .map((entry) => (typeof entry.id === 'string' ? entry.id : null))
        .filter((id) => id && !id.startsWith('__synthetic:')));
    const pollSelectedTitleSet = new Set(pollSelectedOptions
    ...
    const pollOptionTotals = metadataPoll?.optionTotals ??
        (pollAggregates && typeof pollAggregates === 'object' ? pollAggregates.optionTotals : null) ??
        interactivePoll?.optionTotals ??
        null;
    const pollTotalVotes = typeof metadataPoll?.totalVotes === 'number'
        ...
        : typeof pollAggregates?.totalVotes === 'number'
            ...
            : typeof interactivePoll?.totalVotes === 'number'
                ...
                : null;
    const pollTotalVoters = typeof metadataPoll?.totalVoters === 'number'
        ...
        : typeof pollAggregates?.totalVoters === 'number'
            ...
            : typeof interactivePoll?.totalVoters === 'number'
                ...
                : null;
    const pollId = getFirstNonEmptyString(metadataPoll?.pollId, metadataPoll?.id, pollChoiceMetadata?.pollId, pollChoiceMetadata?.id);
    const pollQuestion = getFirstNonEmptyString(metadataPoll?.question, metadataPoll?.title, metadataPoll?.name, interactivePoll?.question, interactivePoll?.title, interactivePoll?.name);
    const pollUpdatedAtIso = getFirstNonEmptyString(metadataPoll?.updatedAt, metadataPoll?.timestamp, pollChoiceMetadata?.vote?.timestamp);
    const pollOptionsSource = Array.isArray(metadataPoll?.options) && metadataPoll.options.length > 0
        ...
        : Array.isArray(interactivePoll?.options)
            ...
            : [];
    const normalizedPollOptions = normalizePollOptions(pollOptionsSource, pollOptionsLookup, pollOptionTotals, pollSelectedIdSet, pollSelectedTitleSet);
