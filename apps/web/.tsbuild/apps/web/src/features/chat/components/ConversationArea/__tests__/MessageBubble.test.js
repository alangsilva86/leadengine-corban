...
describe('MessageBubble', () => {
    ...
    it('renderiza o optionName selecionado quando não há título na metadata', async () => {
        ...
        render(_jsx(MessageBubble, { message: {
                id: 'message-option-name-only',
                direction: 'inbound',
                status: 'SENT',
                type: 'poll_update',
                text: '[Mensagem]',
            }, isContinuation: false, isTail: true, isFirst: true, showMetadata: false }));
    ...
    it('exibe conteúdo de texto padrão quando o hook não sinaliza enquete', () => {
        ...
        render(_jsx(MessageBubble, { message: message, isContinuation: false, isTail: true, isFirst: true, showMetadata: false }));
