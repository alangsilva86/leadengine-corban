...
const SplitLayout = ({ list, detail, listPosition = 'left', className, listClassName, detailClassName, minListWidth = 320, maxListWidthPx = 560, maxListWidthToken = '34vw', listWidth, isListVisible = true, onListWidthChange, onListWidthCommit, resizable = true, ...props }) => {
    const numericWidth = useMemo(() => clampWidth(Number(listWidth), minListWidth, maxListWidthPx), [
        listWidth,
        minListWidth,
        maxListWidthPx,
    ]);
    const fallbackWidth = useMemo(() => `min(${maxListWidthToken}, ${maxListWidthPx}px)`, [maxListWidthToken, maxListWidthPx]);
    ...
    return (_jsxs("div", { className: cn('relative grid h-full min-h-0 w-full gap-0 items-stretch', className), style: { gridTemplateColumns, gridTemplateAreas, gridTemplateRows: '1fr' }, "data-list-position": listPosition, ...props, children: [isListVisible ? (_jsx("aside", { "aria-label": "Lista de tickets", className: cn('relative flex h-full min-h-0 min-w-0 flex-col', listClassName), style: { gridArea: 'list' }, children: list })) : null, _jsx("section", { className: cn('relative flex h-full min-h-0 min-w-0 flex-col', detailClassName), style: { gridArea: 'detail' }, children: detail }), isListVisible && resizable && onListWidthChange ? (_jsx("div", { role: "separator", "aria-label": "Ajustar largura da lista", "aria-orientation": "vertical", "aria-valuemin": minListWidth, "aria-valuemax": maxListWidthPx, "aria-valuenow": numericWidth ?? minListWidth, tabIndex: 0, onPointerDown: handleResizerPointerDown, onKeyDown: handleResizerKeyDown, className: cn('absolute inset-y-0 z-20 w-2 cursor-col-resize touch-none outline-none transition-colors duration-150 focus-visible:ring-2 focus-visible:ring-[color:var(--ring-shell)] focus-visible:ring-offset-2 focus-visible:ring-offset-[color:var(--surface-shell)]', listPosition === 'left' ? '-right-1 translate-x-1/2' : '-left-1 -translate-x-1/2', isDragging
                    ? 'bg-[color:color-mix(in_srgb,var(--border-shell)_55%,transparent)]'
                    : 'bg-transparent'), children: _jsx("span", { "aria-hidden": "true", className: "absolute inset-y-[30%] left-1/2 w-[3px] -translate-x-1/2 rounded-full bg-[color:color-mix(in_srgb,var(--border-shell)_75%,transparent)]" }) })) : null] }));
};
