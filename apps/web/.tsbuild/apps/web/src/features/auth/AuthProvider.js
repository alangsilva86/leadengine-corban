...
import { clearAuthToken, getAuthToken, getTenantId, onAuthTokenChange, onAuthTokenExpire, setAuthToken, setTenantId, setTenantSlugHint, } from '@/lib/auth.js';
...
export const AuthProvider = ({ children }) => {
    ...
    const login = useCallback(async ({ email, password, tenantSlug, remember = false }) => {
        ...
        const tenantSlugHint = payload?.data?.user?.tenant?.slug ?? payload?.data?.user?.tenant?.name ?? tenantSlug ?? null;
        if (tenant) {
            setTenantId(tenant);
        }
        if (tenantSlugHint) {
