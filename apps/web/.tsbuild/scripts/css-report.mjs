...
function limitOffenders(offenders = {}, limit = 5) {
    return Object.fromEntries(Object.entries(offenders).map(([metric, items]) => [metric, items.slice(0, limit)]));
}
async function analyzeWithSourceMapExplorer(cssFiles) {
    ...
    const bundleSummaries = await Promise.all(bundles.map(async (bundle) => {
...
async function main() {
    ...
    if (analysis.totalGzipBytes > budgetBytes) {
        console.error(`Orçamento excedido: ${formatKb(analysis.totalGzipBytes)} kB > ${budgetKb} kB. Revise o CSS antes de prosseguir.`);
        process.exitCode = 1;
    }
}
await main();
