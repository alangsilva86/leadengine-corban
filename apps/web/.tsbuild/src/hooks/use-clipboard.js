...
export const useClipboard = () => {
    ...
    const copy = useCallback(async (rawValue, { emptyMessage = 'Nenhum dado disponível para copiar.', successMessage = 'Copiado para a área de transferência.', errorMessage = 'Não foi possível copiar. Tente novamente.', fallbackMessage, onFallback, } = {}) => {
        ...
        if (success) {
            if (successMessage) {
                showToast('success', typeof successMessage === 'function' ? successMessage(value) : successMessage, toastStateRef);
            }
            return true;
        }
        if (errorMessage) {
        ...
        if (typeof onFallback === 'function') {
            ...
            showToast('info', typeof fallbackMessage === 'function' ? fallbackMessage(value) : fallbackMessage, toastStateRef);
        }
        return false;
    }, [toastStateRef]);
    return { copy };
