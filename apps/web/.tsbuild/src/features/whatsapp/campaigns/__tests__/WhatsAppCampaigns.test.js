...
vi.mock('../../components/CampaignsPanel.jsx', () => ({
    __esModule: true,
    default: (props) => (_jsxs("div", { "data-testid": "campaigns-panel", children: [_jsx("button", { type: "button", onClick: props.onCreateClick, disabled: !props.canCreateCampaigns, children: "Nova campanha" }), _jsx("button", { type: "button", onClick: () => props.onReassign(stubCampaign), children: "Reatribuir campanha" }), _jsx("button", { type: "button", onClick: () => props.onDisconnect(stubCampaign), children: "Desconectar campanha" })] })),
}));
vi.mock('../../components/CreateCampaignDialog.jsx', () => ({
    __esModule: true,
    default: ({ open, onOpenChange }) => open ? (_jsxs("div", { "data-testid": "create-campaign-dialog", children: ["Criar campanha", _jsx("button", { type: "button", onClick: () => onOpenChange(false), children: "Fechar" })] })) : null,
}));
vi.mock('../../components/ReassignCampaignDialog.jsx', () => ({
    __esModule: true,
    default: ({ open, onClose }) => open ? (_jsxs("div", { "data-testid": "reassign-campaign-dialog", children: ["Reatribuir campanha", _jsx("button", { type: "button", onClick: () => onClose(false), children: "Cancelar" })] })) : null,
}));
describe('WhatsAppCampaigns', () => {
    ...
    it('renders the campaigns dashboard and handles actions', async () => {
        render(_jsx(Suspense, { fallback: _jsx("span", { children: "loading" }), children: _jsx(WhatsAppCampaigns, {}) }));
    ...
    it('navigates between stages when the stepper is clicked', async () => {
        render(_jsx(Suspense, { fallback: _jsx("span", { children: "loading" }), children: _jsx(WhatsAppCampaigns, { onboarding: {
                    stages: [
    ...
    it('shows a warning and disables actions when the connection is offline', async () => {
        ...
        render(_jsx(Suspense, { fallback: _jsx("span", { children: "loading" }), children: _jsx(WhatsAppCampaigns, {}) }));
        expect(await screen.findByText('Tempo real está offline. Reative a conexão para gerenciar as campanhas.')).toBeInTheDocument();
    ...
    it('keeps creation enabled when realtime is offline but the instance remains connected', async () => {
        ...
        render(_jsx(Suspense, { fallback: _jsx("span", { children: "loading" }), children: _jsx(WhatsAppCampaigns, {}) }));
        expect(await screen.findByText('Tempo real está offline. Você ainda pode criar ou ajustar campanhas, mas métricas instantâneas ficarão indisponíveis até restabelecer a conexão.')).toBeInTheDocument();
