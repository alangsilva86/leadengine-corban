...
import { ALL_FILTER_VALUE, NO_AGREEMENT_VALUE, NO_INSTANCE_VALUE, } from '../constants.js';
...
export const useCampaignFilters = ({ campaigns, selectedAgreementId }) => {
    ...
    const matchesAgreement = useCallback((campaign) => {
        if (agreementFilter === ALL_FILTER_VALUE) {
            return true;
        }
    ...
    const availableInstances = useMemo(() => {
        const source = agreementFilter === ALL_FILTER_VALUE
            ? campaigns
    ...
    const isFiltered = agreementFilter !== ALL_FILTER_VALUE || instanceFilter !== ALL_FILTER_VALUE;
    return {
        agreementFilter,
        availableAgreements,
        availableInstances,
