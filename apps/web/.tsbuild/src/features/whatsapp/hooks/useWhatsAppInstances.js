...
import { InstancesStoreProvider, createInstancesStore, useInstancesStore, useInstancesStoreBundle, } from '../state/instancesStore';
...
const ProviderEffects = ({ controller, logger }) => {
    ...
    const currentInstanceId = useInstancesStore((state) => state.currentInstance?.id ?? state.preferredInstanceId ?? null);
...
export const WhatsAppInstancesProvider = ({ children, logger: loggerProp, ...providerConfigProps }) => {
    ...
    return (_jsx(InstancesStoreProvider, { value: bundle, children: _jsxs(ServicesContext.Provider, { value: servicesValue, children: [_jsx(ProviderEffects, { controller: servicesValue, logger: logger }), children] }) }));
};
...
export default function useWhatsAppInstances(options = {}) {
    const { selectedAgreement = null, onStatusChange, onError, autoGenerateQr: autoGenerateQrOption, autoRefresh: autoRefreshOption, pauseWhenHidden: pauseWhenHiddenOption, initialFetch: initialFetchOption, } = options;
    ...
    useEffect(() => {
        const desiredConfig = {
            ...
            autoGenerateQr: typeof autoGenerateQrOption === 'boolean'
                ? autoGenerateQrOption
                : controller.providerConfig?.autoGenerateQr ?? true,
            autoRefresh: typeof autoRefreshOption === 'boolean'
                ? autoRefreshOption
                : controller.providerConfig?.autoRefresh ?? false,
            pauseWhenHidden: typeof pauseWhenHiddenOption === 'boolean'
                ? pauseWhenHiddenOption
                : controller.providerConfig?.pauseWhenHidden ?? true,
            initialFetch: typeof initialFetchOption === 'boolean'
                ? initialFetchOption
    ...
    const setSeconds = useCallback((value) => {
        store.getState().setSecondsLeft(value);
    }, [store]);
    const setQr = useCallback((value) => {
        store.getState().setQrData(value);
    }, [store]);
    const markRateLimit = useCallback((timestamp) => {
        store.getState().markRateLimitUntil(timestamp);
    }, [store]);
    const markForcedAt = useCallback((timestamp) => {
        store.getState().markForcedAt(timestamp);
    }, [store]);
    const applyErrorMessage = useCallback((error, fallback) => {
    ...
    const loadInstances = useCallback(async (loadOptions = {}) => {
        const now = Date.now();
        ...
        const requestOptions = {
            ...loadOptions,
            preferredInstanceId: loadOptions.preferredInstanceId ?? preferredInstanceId ?? currentInstance?.id ?? null,
        ...
        if (result.success) {
            ...
            const friendly = applyErrorMessage(result.error, 'Não foi possível carregar status do WhatsApp');
    ...
    const selectInstance = useCallback((id, options = {}) => {
    ...
    const connectInstance = useCallback(async (instanceId, pairingOptions = {}) => {
        if (!instanceId) {
        ...
        try {
            ...
            const friendly = applyErrorMessage(err, 'Não foi possível conectar a instância. Tente novamente em instantes.');
    ...
    const generateQr = useCallback(async (instanceId, options = {}) => {
        if (!instanceId) {
    ...
    const handleAuthFallback = useCallback((fallbackOptions = {}) => {
        store.getState().handleAuthFallback(fallbackOptions);
    }, [store]);
    const setSessionActive = useCallback((value) => {
        store.getState().setSessionActive(Boolean(value));
    }, [store]);
    const setAuthDeferred = useCallback((value) => {
    ...
    const createInstance = useCallback(async ({ name, id }) => {
        if (!name || !name.trim()) {
        ...
        if (!selectedAgreement?.tenantId) {
            const message = 'Selecione um acordo com tenantId válido para criar um novo canal do WhatsApp.';
        ...
        try {
            ...
            const friendly = applyErrorMessage(err, 'Instância não foi provisionada. Tente novamente em instantes.');
    ...
    const deleteInstance = useCallback(async (instance) => {
        ...
        try {
            ...
            toast.success(instance.id.endsWith('@s.whatsapp.net')
                ? 'Sessão desconectada com sucesso'
                : 'Instância removida com sucesso');
        }
        catch (err) {
            const friendly = applyErrorMessage(err, 'Não foi possível remover a instância. Tente novamente em instantes.');
...
export { RATE_LIMIT_COOLDOWN_MS, readInstancesCache, persistInstancesCache, clearInstancesCache, parseInstancesPayload, };
