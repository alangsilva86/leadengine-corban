...
const readErrorMessage = (error) => {
    ...
    if (typeof error === 'object' && error) {
        ...
        const detailMessage = error.payload?.error?.details
            ?.message;
...
const readErrorPayload = (error) => {
    ...
    const responsePayload = error?.response?.data?.error;
...
const normalizeCreateBody = (payload) => {
...
const extractInstanceId = (parsed) => {
...
const withPreferredLoad = (options = {}, preferredInstanceId) => ({
    ...options,
...
const withExponentialBackoff = async (task, { attempts = 3, baseDelayMs = 500, onRetry, } = {}) => {
    let lastError;
...
export const createInstancesApiService = ({ store, events, api, logger, getAuthToken, }) => {
    const log = logger?.log ?? (() => { });
    const warn = logger?.warn ?? (() => { });
    const errorLog = logger?.error ?? (() => { });
    const runLoadRequest = async ({ requestId, options = {}, emitOnly = false, }) => {
        const resolvedRequestId = typeof requestId === 'number'
            ? requestId
        ...
        try {
            ...
            const refreshRequest = () => withExponentialBackoff(() => api.get(`${BASE_PATH}?refresh=1`), {
                attempts: 3,
                baseDelayMs: 500,
                onRetry: ({ attempt, delay, error: retryError }) => warn('Repetindo refresh de instâncias com backoff', {
                    attempt,
                    delay,
                    error: retryError,
                }),
            ...
            if (cachedInstances.length > 0) {
                ...
                store.getState().applyLoadResult({
                    ...
                    connected: cachedState.status === 'connected'
                        ? true
            ...
            const delayMs = err?.rateLimitDelayMs;
    ...
    const markConnected = async ({ instanceId }) => {
        try {
            const encodedId = encodeURIComponent(instanceId);
            const response = await api.get(`${BASE_PATH}/${encodedId}/status`);
            const parsed = parseInstancesPayload(response);
            const connected = typeof parsed.connected === 'boolean'
                ? parsed.connected
