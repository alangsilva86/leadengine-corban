...
const useWhatsappPairing = ({ state, setPairingPhoneInput, setPairingPhoneError, setRequestingPairing, instanceId, selectedAgreementId, connectInstance, loadInstances, setErrorMessage, }) => {
    useEffect(() => {
    ...
    const resolveFriendlyError = useCallback((error, fallbackMessage) => {
        ...
        const rawMessage = error?.payload?.error?.message ?? (error instanceof Error ? error.message : fallbackMessage);
    ...
    const handleRequestPairingCode = useCallback(async () => {
        ...
        try {
            ...
            toast.success('Solicitamos o código de pareamento. Abra o WhatsApp oficial e informe o código recebido para concluir a conexão.');
        }
        catch (err) {
            const friendly = resolveFriendlyError(err, 'Não foi possível solicitar o pareamento por código. Verifique o telefone informado e tente novamente.');
