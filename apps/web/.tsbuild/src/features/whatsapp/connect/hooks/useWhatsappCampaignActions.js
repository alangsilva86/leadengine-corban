...
import { createCampaign as createCampaignRequest, deleteCampaign as deleteCampaignRequest, fetchCampaigns, fetchCampaignImpact, reassignCampaign as reassignCampaignRequest, updateCampaignStatus as updateCampaignStatusRequest, } from '../services/campaignService';
const useWhatsappCampaignActions = ({ state, dispatch, selectedAgreement, activeCampaign, instance, instances, handleAuthFallback, logError, onCampaignReady, }) => {
    useEffect(() => {
    ...
    const loadCampaignsRef = useRef(() => Promise.resolve());
    const loadCampaigns = useCallback(async ({ preferredAgreementId, preferredCampaignId, preferredInstanceId, } = {}) => {
        ...
        try {
            ...
            const message = err?.payload?.error?.message ||
    ...
    const createCampaign = useCallback(async ({ name, instanceId, agreementId, agreementName, leadSource, product, margin, strategy, status: requestedStatus = 'active', segments, }) => {
        ...
        const brokerId = targetInstance && targetInstance.metadata && typeof targetInstance.metadata === 'object'
    ...
    const updateCampaignStatus = useCallback(async (target, nextStatus) => {
        if (!target?.id) {
            return;
        }
        dispatch({ type: 'set-campaign-error', value: null });
    ...
    const deleteCampaign = useCallback(async (target) => {
        if (!target?.id) {
            return;
        }
        dispatch({ type: 'set-campaign-error', value: null });
    ...
    const reassignCampaign = useCallback(async (target, requestedInstanceId) => {
        if (!target?.id) {
            return;
        }
        dispatch({ type: 'set-campaign-error', value: null });
        ...
        try {
            ...
            toast.success(requestedInstanceId
    ...
    const setCreateCampaignOpen = useCallback((value) => dispatch({ type: 'set-create-campaign-open', value }), [dispatch]);
    const setPendingReassign = useCallback((value) => dispatch({ type: 'set-pending-reassign', value }), [dispatch]);
    const setReassignIntent = useCallback((value) => dispatch({ type: 'set-reassign-intent', value }), [dispatch]);
