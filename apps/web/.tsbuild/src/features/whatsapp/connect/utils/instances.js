...
const resolveInstanceId = (target) => {
    if (!target)
        return null;
...
const isInstanceConnected = (entry) => {
...
const buildInstanceViewModels = (instances, currentInstance) => {
    return instances.map((entry, index) => {
        ...
        const addressCandidate = (typeof entry?.address === 'string' && entry.address) ||
        ...
        return {
            ...
            displayName: (typeof entry?.name === 'string' && entry.name) ||
            ...
            user,
            instance: entry,
            isCurrent: Boolean(currentInstance?.id && entry?.id && currentInstance.id === entry.id) ||
...
const sortInstancesByLabel = (instances) => {
