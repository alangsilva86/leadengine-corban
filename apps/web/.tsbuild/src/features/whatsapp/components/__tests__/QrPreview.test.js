...
const { passthrough } = vi.hoisted(() => {
    const factory = (Tag = 'div') => {
        const Component = ({ children, ...props }) => _jsx(Tag, { ...props, children: children });
        return Component;
    };
...
vi.mock('lucide-react', () => ({
    Clock: (props) => _jsx("svg", { "data-testid": "clock-icon", ...props }),
    Loader2: (props) => _jsx("svg", { "data-testid": "loader-icon", ...props }),
    QrCode: (props) => _jsx("svg", { "data-testid": "qr-icon", ...props }),
    RefreshCcw: (props) => _jsx("svg", { "data-testid": "refresh-icon", ...props }),
}));
vi.mock('prop-types', () => ({
    __esModule: true,
    default: new Proxy({}, {
...
describe('QrPreview', () => {
    it('renderiza a imagem quando o QR está disponível e dispara ações', async () => {
        ...
        render(_jsx(QrPreview, { src: "data:image/png;base64,AAAA", statusMessage: "QR v\u00E1lido por 1 minuto", onGenerate: onGenerate, onOpen: onOpen }));
    ...
    it('exibe placeholder ou loader quando o QR ainda não foi carregado', () => {
        const { rerender, container } = render(_jsx(QrPreview, {}));
        ...
        rerender(_jsx(QrPreview, { isGenerating: true }));
    ...
    it('respeita estados desabilitados dos botões e mantém acessibilidade do status', async () => {
        ...
        const onOpen = vi.fn();
        const user = userEvent.setup();
        render(_jsx(QrPreview, { statusMessage: "Gerando novo c\u00F3digo", onGenerate: onGenerate, onOpen: onOpen, generateDisabled: true, openDisabled: true }));
