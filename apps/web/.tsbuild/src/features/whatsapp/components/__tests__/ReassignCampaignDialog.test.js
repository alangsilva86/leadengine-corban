...
describe('ReassignCampaignDialog', () => {
    it('permite desvincular a campanha enviando instanceId null', async () => {
        ...
        render(_jsx(ReassignCampaignDialog, { open: true, campaign: baseCampaign, instances: instances, onClose: vi.fn(), onSubmit: onSubmit, fetchImpact: vi.fn(() => Promise.resolve({ summary: null })) }));
    ...
    it('preseleciona desvincular quando intent é disconnect', async () => {
        render(_jsx(ReassignCampaignDialog, { open: true, campaign: { ...baseCampaign, instanceId: 'instance-1' }, instances: instances, onClose: vi.fn(), onSubmit: vi.fn(), fetchImpact: vi.fn(() => Promise.resolve({ summary: null })), intent: "disconnect" }));
        const trigger = screen.getByRole('combobox');
        expect(within(trigger).getByText(/Sem instância \(aguardando vínculo\)/i)).toBeInTheDocument();
    });
});
