...
const normalizeDescriptor = (payload = {}, fallback = {}) => {
    ...
    const fileName = typeof payload.fileName === 'string'
        ...
        : typeof payload.mediaFileName === 'string'
            ...
            : fallback.fileName;
    const size = typeof payload.size === 'number'
        ? payload.size
...
export const useUploadWhatsAppMedia = () => {
    return useMutation({
        ...
        mutationFn: async ({ file, fileName, mimeType } = {}) => {
            ...
            const inferredName = fileName || (typeof file.name === 'string' && file.name.length > 0 ? file.name : undefined);
            const inferredMime = mimeType || (typeof file.type === 'string' && file.type.length > 0 ? file.type : undefined);
