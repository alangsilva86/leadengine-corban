...
import useUpdateContactField from '../api/useUpdateContactField';
import useUpdateDealFields from '../api/useUpdateDealFields.js';
import useUpdateNextStep from '../api/useUpdateNextStep.js';
import { contactFieldUpdateSchema, dealFieldUpdateSchema, normalizeContactFieldValue, normalizeDealFieldValue, } from '../utils/validation';
...
export const useTicketFieldUpdaters = ({ controller, selectedTicket, selectedContact, selectedLead, currentUser, }) => {
    ...
    const updateContactFieldMutation = useUpdateContactField(contactId ? { contactId } : undefined);
    const updateNextStepMutation = useUpdateNextStep(ticketIdFromSelection ? { ticketId: ticketIdFromSelection } : undefined);
    const updateDealFieldsMutation = useUpdateDealFields(leadId ? { leadId } : undefined);
    const [nextStepDraft, setNextStepDraft] = useState('');
    useEffect(() => {
        const nextStep = selectedTicket?.metadata?.nextAction?.description ??
    ...
    const handleContactFieldSave = useCallback(async (field, rawValue) => {
    ...
    const handleUpdateNextStep = useCallback(async (value) => {
        ...
        const currentDescription = selectedTicket?.metadata?.nextAction?.description ??
            selectedTicket?.nextStep?.description ??
            '';
    ...
    const handleDealFieldSave = useCallback(async (field, rawValue) => {
        if (!selectedLead?.id) {
        ...
        const currentDeal = selectedLead?.customFields && typeof selectedLead.customFields === 'object'
