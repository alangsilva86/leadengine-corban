...
const ensureContactForPhone = async ({ digits, phoneE164 }) => {
    ...
    try {
        ...
        if (error?.status === 409) {
            ...
            if (retry?.id) {
                const normalizedPhone = typeof retry.phone === 'string' && retry.phone.trim().length > 0 ? retry.phone : fallbackE164;
    ...
    const mutation = useMutation({
        ...
        mutationFn: async ({ phone, message, instanceId }) => {
            ...
            if (!validation.payload) {
                const fallbackMessage = validation.errors?.phone ?? validation.errors?.message ?? validation.errors?.instanceId ?? 'Dados inválidos.';
            ...
            const outboundResponse = await apiPost(`/api/contacts/${contact.id}/messages`, {
                payload: {
                    type: 'text',
                    text: trimmedMessage,
                },
