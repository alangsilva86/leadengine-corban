...
describe('useManualConversationLauncher', () => {
    ...
    it('envia o payload formatado e retorna o ticket criado', async () => {
        ...
        expect(apiPostMock).toHaveBeenCalledWith('/api/contacts/contact-123/messages', expect.objectContaining({
    ...
    it('cria o contato quando necessário antes de enviar', async () => {
        ...
        expect(apiPostMock).toHaveBeenNthCalledWith(1, '/api/contacts', { name: '+5531988887777', phone: '+5531988887777' });
        expect(apiPostMock).toHaveBeenNthCalledWith(2, '/api/contacts/contact-new/messages', expect.objectContaining({
    ...
    it('normaliza JIDs do WhatsApp antes de enviar', async () => {
        ...
        expect(apiGetMock).toHaveBeenCalledWith(expect.stringContaining('/api/contacts?limit=5&search=5511999998888'));
        expect(apiPostMock).toHaveBeenCalledWith('/api/contacts/contact-123/messages', expect.objectContaining({ to: '+5511999998888', instanceId: 'instance-jid' }), expect.any(Object));
    });
    it('falha ao enviar quando faltam dados obrigatórios', async () => {
        const { result } = renderHook(() => useManualConversationLauncher(), {
            wrapper: createWrapper(),
        });
        await expect(result.current.launch({ phone: '', message: 'hi', instanceId: 'abc' })).rejects.toThrow('Informe um telefone válido com DDD e país.');
        await expect(result.current.launch({ phone: '12345', message: 'hi', instanceId: 'abc' })).rejects.toThrow('Informe um telefone válido com DDD e país.');
        await expect(result.current.launch({ phone: '(11) 90000-0000', message: '   ', instanceId: 'abc' })).rejects.toThrow('Digite a mensagem inicial.');
        await expect(result.current.launch({ phone: '(11) 90000-0000', message: 'olá', instanceId: '' })).rejects.toThrow('Selecione uma instância conectada.');
    });
