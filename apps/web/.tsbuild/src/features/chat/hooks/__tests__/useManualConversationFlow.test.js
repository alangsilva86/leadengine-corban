...
describe('useManualConversationFlow', () => {
    let useManualConversationFlowHook;
    beforeEach(async () => {
    ...
    it('notifies error when launcher is unavailable', async () => {
        ...
        const controller = {};
        const { result } = renderHook(() => useManualConversationFlowHook({ controller }));
        await expect(result.current.onSubmit({ phone: '123', message: 'Olá', instanceId: 'iid-1' })).rejects.toThrow('Indisponível');
    ...
    it('propagates launcher errors with contextual toast', async () => {
        ...
        const controller = {};
        const { result } = renderHook(() => useManualConversationFlowHook({ controller }));
        await expect(result.current.onSubmit({ phone: '123', message: 'Olá', instanceId: 'iid-1' })).rejects.toThrow('failure');
