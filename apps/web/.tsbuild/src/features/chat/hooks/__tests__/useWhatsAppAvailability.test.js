...
describe('useWhatsAppAvailability', () => {
    ...
    it('stores broker unavailability and surfaces toast', () => {
        ...
        act(() => {
            result.current.notifyOutboundError({ code: 'BROKER_NOT_CONFIGURED' }, 'fallback');
        });
    ...
    it('resets availability state on demand', () => {
        const { result } = renderHook(() => useWhatsAppAvailability({ selectedTicketId: 'ticket-1' }));
        act(() => {
            result.current.notifyOutboundError({ code: 'UNKNOWN' }, 'fallback');
        });
        act(() => {
    ...
    it('clears unavailability when ticket changes', () => {
        const { result, rerender } = renderHook(({ ticketId }) => useWhatsAppAvailability({ selectedTicketId: ticketId }), { initialProps: { ticketId: 'ticket-1' } });
        act(() => {
            result.current.notifyOutboundError({ code: 'BROKER_NOT_CONFIGURED' }, 'fallback');
        });
