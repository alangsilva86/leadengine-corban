...
describe('ConversationArea', () => {
    ...
    it('renders with minimal ticket and handlers', () => {
        ...
        const { getByText } = render(_jsx(QueryClientProvider, { client: queryClient, children: _jsx(ConversationArea, { ticket: ticket, conversation: conversation, messagesQuery: messagesQuery, onSendMessage: () => { }, onCreateNote: () => { }, onSendTemplate: () => { }, onCreateNextStep: () => { }, onRegisterResult: () => { }, onRegisterCallResult: () => { }, onAssign: () => { }, onGenerateProposal: () => { }, onScheduleFollowUp: () => { }, onSendSMS: () => { }, onEditContact: () => { }, onAttachFile: () => { }, typingIndicator: typingIndicator }) }));
    ...
    it('usa a sanitização compartilhada ao iniciar a geração de resposta da IA', async () => {
        ...
        const { result } = renderHook(() => useConversationExperience({
            ticket,
