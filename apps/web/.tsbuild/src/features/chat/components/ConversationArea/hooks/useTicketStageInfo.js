...
const useTicketStageInfo = (ticket) => {
    const stageKey = useMemo(() => getTicketStage(ticket), [ticket?.metadata?.pipelineStep, ticket?.pipelineStep, ticket?.stage]);
    const stageInfo = useMemo(() => getStageInfo(stageKey), [stageKey]);
    const primaryAction = useMemo(() => {
        if (!ticket) {
            return null;
        }
        const hasPhone = Boolean(ticket?.contact?.phone ??
            (Array.isArray(ticket?.contact?.phones) && ticket.contact.phones.length > 0) ??
            ticket?.metadata?.contactPhone);
        const whatsappChannel = ticket?.metadata?.channels?.whatsapp ??
            ticket?.channels?.whatsapp ??
            null;
        const whatsappIsInvalid = (typeof whatsappChannel?.valid === 'boolean' && whatsappChannel.valid === false) ||
