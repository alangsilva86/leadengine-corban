...
export const useConversationScroll = ({ scrollRef, ticketId, lastEntryKey, typingAgentsCount, scrollToBottom, onLoadMore, }) => {
    useEffect(() => {
        scrollToBottom();
    ...
    useEffect(() => {
        ...
        const schedule = hasWindow && typeof window.requestAnimationFrame === 'function'
            ? window.requestAnimationFrame.bind(window)
            : (cb) => setTimeout(cb, 16);
        const cancelFn = hasWindow && typeof window.cancelAnimationFrame === 'function'
