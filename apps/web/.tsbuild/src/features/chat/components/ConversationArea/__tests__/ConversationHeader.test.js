...
vi.mock('../QuickComposer.jsx', () => ({
    __esModule: true,
    default: () => _jsx("div", { "data-testid": "quick-composer", children: "QuickComposer" }),
}));
vi.mock('../CallResultDialog.jsx', () => ({
    __esModule: true,
    default: () => _jsx("div", { "data-testid": "call-result-dialog", children: "CallResultDialog" }),
}));
vi.mock('../LossReasonDialog.jsx', () => ({
    __esModule: true,
    default: () => _jsx("div", { "data-testid": "loss-reason-dialog", children: "LossReasonDialog" }),
}));
vi.mock('../CommandBar.jsx', () => ({
    __esModule: true,
    CommandBar: () => _jsx("div", { "data-testid": "command-bar" }),
}));
...
describe('ConversationHeader helpers', () => {
    ...
    it('renders the primary action button when an action is provided', () => {
        STAGE_SCENARIOS.forEach(({ key, hasWhatsApp }) => {
            const action = resolvePrimaryAction({ stageKey: key, hasWhatsApp, needsContactValidation: false });
            const { unmount } = render(_jsx(PrimaryActionButton, { action: action, jroState: "neutral", onExecute: () => { }, disabled: false }));
    ...
    it('renderiza um indicador acessível para cada etapa do funil', () => {
        STAGE_SCENARIOS.forEach(({ raw, key }) => {
            ...
            const { unmount } = render(_jsx(ConversationHeader, { ticket: ticket }));
    ...
    it('abre o popover de etapas ao acionar o chip da etapa atual', async () => {
        ...
        render(_jsx(ConversationHeader, { ticket: ticket }));
...
describe('ContactSummary channel icons', () => {
    ...
    const renderHeaderWithChannel = async (channel) => {
        ...
        render(_jsx(ConversationHeader, { ticket: ticket, typingAgents: [], onAssign: () => { }, onScheduleFollowUp: () => { }, onRegisterResult: () => { }, onRegisterCallResult: () => { }, onSendTemplate: () => { }, onCreateNextStep: () => { }, onGenerateProposal: () => { }, onSendSMS: () => { }, onAttachFile: () => { } }));
        const user = userEvent.setup();
...
describe('ConversationHeader component', () => {
    ...
    it('exibe o modo de IA ativo no botão compacto', () => {
        ...
        render(_jsx(ConversationHeader, { ticket: ticket, aiMode: "manual", aiConfidence: 0.82 }));
    ...
    it('propaga callbacks do menu de IA', async () => {
        ...
        render(_jsx(ConversationHeader, { ticket: ticket, aiConfidence: 0.85, onAiModeChange: onAiModeChange, onTakeOver: onTakeOver, onGiveBackToAi: onGiveBackToAi }));
        const user = userEvent.setup();
    ...
    it('renders liquidation panel only for liquidation stages', async () => {
        ...
        const { rerender } = render(_jsx(ConversationHeader, { ticket: ticket, onDealFieldSave: () => { }, renderSummary: (summary, helpers) => (_jsxs("div", { children: [summary, _jsx("button", { type: "button", onClick: () => helpers.onOpenChange(true), children: openLabel })] })) }));
        ...
        rerender(_jsx(ConversationHeader, { ticket: { ...ticket, pipelineStep: 'Proposta' }, onDealFieldSave: () => { }, renderSummary: (summary, helpers) => (_jsxs("div", { children: [summary, _jsx("button", { type: "button", onClick: () => helpers.onOpenChange(true), children: openLabel })] })) }));
        await waitFor(() => {
    ...
    it('propagates composer height as a CSS variable when provided', async () => {
        ...
        render(_jsx(ConversationHeader, { ticket: ticket, composerHeight: 264, typingAgents: [], onAssign: () => { }, onScheduleFollowUp: () => { }, onRegisterResult: () => { }, onRegisterCallResult: () => { }, onSendTemplate: () => { }, onCreateNextStep: () => { }, onGenerateProposal: () => { }, onSendSMS: () => { }, onAttachFile: () => { } }));
        const user = userEvent.setup();
        const toggleButton = screen.getByRole('button', { name: /Expandir detalhes/i });
        await user.click(toggleButton);
