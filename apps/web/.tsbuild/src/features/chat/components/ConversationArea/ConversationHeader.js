...
const getOriginLabel = (ticket) => {
    const origin = ticket?.metadata?.origin ??
...
const ConversationHeader = ({ ticket, onRegisterResult, onRegisterCallResult, onAssign, onSendTemplate, onCreateNextStep, onGenerateProposal, onScheduleFollowUp, onSendSMS, onAttachFile, onEditContact, typingAgents = [], isRegisteringResult = false, renderSummary, onContactFieldSave, onDealFieldSave, nextStepValue, onNextStepSave, onFocusComposer, composerHeight, aiMode = DEFAULT_AI_MODE, aiConfidence = null, aiModeChangeDisabled = false, onTakeOver, onGiveBackToAi, onAiModeChange, onCreateNote, timeline = [], aiAssistant, components, detailsOpen = false, onRequestDetails, nextStepEditorRef: externalNextStepEditorRef, primaryActionOverride = null, }) => {
    ...
    const { stageKey, stageInfo, primaryAction: derivedPrimaryAction, } = useTicketStageInfo(ticket);
    ...
    const handleCallResultSubmit = useCallback(({ outcome, notes }) => {
    ...
    const handleSuccessSubmit = useCallback(async ({ installment, netAmount, term, product, bank, notes }) => {
        if (!onRegisterResult) {
            toast.error('Não foi possível concluir. Tente novamente.');
            return;
        }
    ...
    const { PrimaryActionBanner: PrimaryActionBannerComponent = PrimaryActionBanner, } = components ?? {};
    const summaryContent = (_jsx(PrimaryActionBannerComponent, { name: name, title: title, shortId: shortId, ticket: ticket, statusInfo: statusInfo, stageKey: stageKey, stageInfo: stageInfo, originInfo: originInfo, typingAgents: typingAgents, primaryAction: primaryAction, onPrimaryAction: handlePrimaryAction, jro: jro, commandContext: commandContext, detailsOpen: detailsOpen, onRequestDetails: onRequestDetails, nextStepValue: nextStepValue, aiMode: aiMode, aiConfidence: aiConfidence, aiModeChangeDisabled: aiModeChangeDisabled, onAiModeChange: onAiModeChange, onTakeOver: onTakeOver, onGiveBackToAi: onGiveBackToAi, contactPhone: identity.displayPhone ?? identity.remoteJid ?? null, instanceId: instanceId, instancePresentation: instancePresentation }));
    const renderedSummary = renderSummary
        ? renderSummary(summaryContent)
        : summaryContent;
    return (_jsxs("section", { className: "relative z-10 flex flex-col gap-3 px-0 py-1.5", children: [renderedSummary, _jsx(OutcomeDialog, { open: activeDialog === 'register-result', mode: outcomeMode, onModeChange: (nextMode) => {
                    if (!nextMode) {
                        closeDialog();
                        return;
                    }
                ...
                }, lossOptions: LOSS_REASONS, onConfirmLoss: handleLossReasonSubmit, onConfirmSuccess: handleSuccessSubmit, isSubmitting: isRegisteringResult }), _jsx(CallResultDialog, { open: activeDialog === 'call-result', onOpenChange: (open) => {
                    if (open) {
