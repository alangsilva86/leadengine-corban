...
const ConversationAreaMock = vi.hoisted(() => {
    const mock = vi.fn((props) => {
        mock.props = props;
        return _jsx("div", { "data-testid": "conversation-area" });
    });
    return mock;
});
...
vi.mock('../components/layout/InboxAppShell.jsx', () => ({
    __esModule: true,
    default: ({ sidebar, toolbar, children }) => (_jsxs("div", { children: [_jsx("div", { children: sidebar }), _jsx("div", { children: toolbar }), _jsx("div", { children: children })] })),
}));
vi.mock('../components/QueueList/QueueList.jsx', () => ({
    __esModule: true,
    default: () => _jsx("div", { "data-testid": "queue-list" }),
}));
vi.mock('../components/FilterToolbar/FilterToolbar.jsx', () => ({
    __esModule: true,
    default: () => _jsx("div", { "data-testid": "filter-toolbar" }),
}));
vi.mock('../components/ManualConversationDialog.jsx', () => ({
    __esModule: true,
    default: () => _jsx("div", { "data-testid": "manual-conversation-dialog" }),
}));
...
describe('ChatCommandCenter next step editor', () => {
    ...
    it('chama a mutação com descrição normalizada e metadata padrão', async () => {
        ...
        render(_jsx(ChatCommandCenter, { tenantId: "tenant-x", currentUser: { id: 'agent-42' } }));
    ...
    it('propaga erros da API e exibe toast de erro', async () => {
        ...
        render(_jsx(ChatCommandCenter, { tenantId: "tenant-x", currentUser: { id: 'agent-42' } }));
        await expect(ConversationAreaMock.props.onNextStepSave?.('Atualizar próxima etapa')).rejects.toThrow('Falha na API');
    ...
    it('reidrata o rascunho quando o ticket selecionado muda', async () => {
        nextStepMutationMock.mutateAsync.mockResolvedValue({ id: 'ticket-1' });
        const { rerender } = render(_jsx(ChatCommandCenter, { tenantId: "tenant-x", currentUser: { id: 'agent-42' } }));
        expect(ConversationAreaMock.props?.nextStepValue).toBe('Falar sobre proposta');
        await act(async () => {
            ...
            rerender(_jsx(ChatCommandCenter, { tenantId: "tenant-x", currentUser: { id: 'agent-42' } }));
        });
        await waitFor(() => {
