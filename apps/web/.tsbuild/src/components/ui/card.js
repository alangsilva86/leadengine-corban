...
function Card({ className = '', ...props }) {
    return (_jsx("div", { "data-slot": "card", className: cn("glass-surface text-card-foreground flex flex-col gap-6 rounded-[var(--radius)] border border-[var(--border)] py-6 shadow-[var(--shadow-lg)]", className), ...props }));
}
...
function CardBody({ className = '', ...props }) {
    return (_jsx("div", { "data-slot": "card-body", className: cn("px-6", className), ...props }));
}
function CardFooter({ className = '', ...props }) {
    return (_jsx("div", { "data-slot": "card-footer", className: cn("flex items-center px-6 [.border-t]:pt-6", className), ...props }));
}
export { Card, CardHeader, CardFooter, CardTitle, CardAction, CardDescription, CardContent, CardBody, };
