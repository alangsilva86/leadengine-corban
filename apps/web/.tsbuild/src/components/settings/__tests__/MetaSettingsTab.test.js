...
const buildFetchMock = () => {
    ...
    const fetchMock = vi.fn(async (input, init) => {
        ...
        if (url.endsWith('/api/integrations/meta/offline-conversions/config') && (!init || init.method === 'GET')) {
            return new Response(JSON.stringify({ success: true, data: state.config }), { status: 200, headers: { 'Content-Type': 'application/json' } });
        }
        if (url.endsWith('/api/integrations/meta/offline-conversions/config') && init?.method === 'PUT') {
            ...
            const nextAccessTokenConfigured = payload.accessToken !== undefined ? Boolean(payload.accessToken) : state.config.accessTokenConfigured;
            const nextAppSecretConfigured = payload.appSecret !== undefined ? Boolean(payload.appSecret) : state.config.appSecretConfigured;
            state.config = {
                ...state.config,
                ...payload,
