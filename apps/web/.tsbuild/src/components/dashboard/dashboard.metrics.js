...
export const deriveTrend = (change) => {
    if (!change || change === '—')
        return 'neutral';
    if (change.startsWith('-'))
        return 'down';
    if (change === '0%' || change === '+0%' || change === '0,0%' || change === '+0,0%')
        return 'neutral';
    return 'up';
};
...
export const buildChannelDistribution = (channelCounts) => {
...
const buildRecentTickets = (tickets) => tickets
    .map((ticket) => {
    if (!ticket)
        return null;
    const channel = normalizeChannel(ticket.channel);
    const timestamp = parseDate(ticket.updatedAt) ?? parseDate(ticket.lastMessageAt) ?? parseDate(ticket.createdAt);
    return {
...
export const processLeads = (leadsData, dashboardMetrics) => {
    ...
    const totalLeads = typeof dashboardMetrics?.totalLeads === 'number' ? dashboardMetrics.totalLeads : totalFromList;
    const convertedFromList = items.filter((lead) => String(lead?.status ?? '').toUpperCase() === 'CONVERTED').length;
    const totalHotLeads = typeof dashboardMetrics?.totalHotLeads === 'number'
...
export const buildDashboardMetrics = (ticketsData, leadsData, dashboardMetrics) => {
