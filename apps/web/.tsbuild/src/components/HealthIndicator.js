...
export default function HealthIndicator({ intervalMs = 30000 }) {
    ...
    const normalizePayload = useCallback((payload) => {
        ...
        const candidateStatus = typeof payload.status === 'string'
            ? payload.status
    ...
    return (_jsxs("div", { className: "inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs", style: { background: palette.bg, color: palette.fg, border: palette.border }, title: JSON.stringify(details), children: [_jsx("span", { "aria-hidden": "true", style: { backgroundColor: palette.dot }, className: "inline-block h-2.5 w-2.5 rounded-full" }), _jsx("span", { children: title })] }));
}
