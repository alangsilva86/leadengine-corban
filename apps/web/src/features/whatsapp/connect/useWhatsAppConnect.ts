...
const shouldDisplayInstance = (instance: any) => {
...
const resolveInstanceStatus = (instance: any) => {
  if (!instance || typeof instance !== 'object') {
    return null;
  }
...
const normalizeKeyName = (value: string) => value.toLowerCase().replace(/[^a-z0-9]+/g, '');
...
const pickMetric = (source: any, keys: string[]) => {
  ...
  for (const key of keys) {
    ...
    for (const [candidateKey, value] of Object.entries(source)) {
      if (normalizeKeyName(candidateKey) === normalized) {
        return value;
      }
    }
  }
...
const findStatusCountsSource = (source: any): any => {
  if (!source || typeof source !== 'object') {
    return undefined;
  }

  const keywords = ['status', 'codes', 'codigo', 'codigos'];
...
const normalizeStatusCounts = (rawCounts: any) => {
...
const findRateSource = (source: any) => {
  if (!source) {
    return undefined;
  }
...
const normalizeRateUsage = (rawRate: any) => {
  const defaults = {
    used: 0,
    limit: 0,
    remaining: 0,
...
const mergeMetricsSources = (...sources: any[]) => {
...
const getInstanceMetrics = (instance: any) => {
...
const getStatusInfo = (instance: any) => {
...
const formatPhoneNumber = (value: unknown) => {
  if (!value) return '—';
...
const resolveInstancePhone = (instance: any) =>
