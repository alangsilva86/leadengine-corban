...
* **Taxa de sucesso de coleta/criação (`whatsapp_instance_operation_total`)** – `Stat` por `operation` com filtro obrigatório de `tenantId` e `mode` (db/snapshot/sync/api). Fórmula sugerida: `1 - (sum(rate(whatsapp_instance_operation_total{operation="collect",result!="success"}[5m])) by (tenantId,mode) / sum(rate(whatsapp_instance_operation_total{operation="collect"}[5m])) by (tenantId,mode))`.
* **Latência P95 de coleta/criação (`whatsapp_instance_operation_duration_ms`)** – `Time series` agrupada por `tenantId`/`mode` com `histogram_quantile(0.95, sum(rate(whatsapp_instance_operation_duration_ms_sum[5m])) by (tenantId,mode,operation) / sum(rate(whatsapp_instance_operation_duration_ms_count[5m])) by (tenantId,mode,operation))`.
...
| `WhatsAppInstanceCollectionSuccessDrop` | `sum(rate(whatsapp_instance_operation_total{operation="collect",result!="success"}[5m])) by (tenantId,mode) / sum(rate(whatsapp_instance_operation_total{operation="collect"}[5m])) by (tenantId,mode) > 0.05` por 5 minutos | Alertar queda de sucesso na coleta de instâncias por tenant e estratégia de busca. |
| `WhatsAppInstanceOperationLatencyP95` | `histogram_quantile(0.95, sum(rate(whatsapp_instance_operation_duration_ms_sum[5m])) by (tenantId,operation,mode) / sum(rate(whatsapp_instance_operation_duration_ms_count[5m])) by (tenantId,operation,mode)) > 3000` por 10 minutos | Detectar aumento de latência (P95) em coleta ou criação de instâncias por tenant. |
