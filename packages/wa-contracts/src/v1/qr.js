const DEFAULT_QR_PAYLOAD = Object.freeze({
    qr: null,
    qrCode: null,
    qrExpiresAt: null,
    expiresAt: null,
...
const mergeQrPayloads = (primary, secondary) => {
    if (!primary)
        return secondary;
    if (!secondary)
        return primary;
...
const parseCandidate = (candidate, visited) => {
    ...
    const directQr = pickString(source.qr, source.qrCode, source.qr_code, source.code, source.image, source.value);
    const qrCodeCandidate = pickString(source.qrCode, source.qr_code, source.code);
    const qrExpiresCandidate = pickString(source.qrExpiresAt, source.qr_expires_at, source.expiresAt, source.expires_at);
    const expiresCandidate = pickString(source.expiresAt, source.expires_at, source.expiration, source.expires, source.expire_at);
    const availableCandidate = pickBoolean(source.available, source.qrAvailable, source.qr_available);
    const reasonCandidate = normalizeReason(pickString(source.reason, source.qrReason, source.qr_reason));
    let normalized = null;
...
const finalizeQrPayload = (payload) => {
    if (!payload) {
...
export const normalizeQrPayload = (value) => {
